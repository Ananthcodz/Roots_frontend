 ...
 * @param {string} rootMemberId - ID of the root member (tree owner)
 * @returns {Object} Tree node structure
 */
export function buildTreeStructure(members, relationships, rootMemberId) {
  ...
  calculateLevels(rootNode, 0, new Set());
 ...
 */
function calculateLevels(node, level, visited) {
 ...
 * @returns {Map} Map of member ID to position {x, y}
 */
export function calculateTreeLayout(rootNode) {
  ...
  collectNodesByLevel(rootNode, levelNodes, new Set());
 ...
 * @param {Object} node - Tree node
 * @param {Map} levelNodes - Map of level to nodes
 * @param {Set} visited - Set of visited node IDs
 */
function collectNodesByLevel(node, levelNodes, visited) {
  if (!node || visited.has(node.member.id)) {
    return;
  }
 ...
 * @param {Array} members - Array of family member objects
 * @param {Object} rootNode - Root tree node
 * @returns {Object} Statistics object with memberCount and generationCount
 */
export function calculateTreeStatistics(members, rootNode) {
  const memberCount = members ? members.length : 0;
  
  let generationCount = 0;
  if (rootNode) {
    const levels = new Set();
    collectLevels(rootNode, levels, new Set());
 ...
 * @param {Object} node - Tree node
 * @param {Set} levels - Set of levels
 * @param {Set} visited - Set of visited node IDs
 */
function collectLevels(node, levels, visited) {
  if (!node || visited.has(node.member.id)) {
    return;
  }
 ...
 * @param {string} startMemberId - Starting member ID
 * @param {string} endMemberId - Target member ID
 * @param {Array} relationships - Array of relationship objects
 * @param {Array} members - Array of family member objects
 * @returns {Array|null} Array of member IDs representing the path, or null if no path exists
 */
export function findRelationshipPath(startMemberId, endMemberId, relationships, members) {
 ...
 * @returns {string} Reverse relationship type
 */
